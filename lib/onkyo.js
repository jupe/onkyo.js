...
{
  this.options = options || {log: false};
  this.destinationPort = 60128;
  this.broadcastPort = 60128;
  this.broadcastAddress = this.options.broadcastAddress || "192.168.1.255";
  this.reqCallback;
  this.detectedDevice;
  if( typeof(this.options.ip) != 'undefined' )
     this.detectedDevice  = {
      address:  this.options.ip,
      port: this.options.port || 60128
     };
  this.client;
  ...
  this.server.on("message", function (msg, rinfo) {
    ...
    if( self.detectedDevice ){
      ...
      if(self.reqCallback) self.reqCallback(null, self.detectedDevice);
...
Onkyo.prototype.ParsePacket = function(packetstring) {
    ...
    switch (group) {
      ...
      case "SLI":
        if((obj=command.match(/([0-9A-Z]{2})/)))
        {
          var lookupValue = "SLI" + command;

          var source = this.GetKeyByValue(this.cmds["SOURCE_SELECT"] , lookupValue);
      ...
      case "LMD":
        if((obj=command.match(/([0-9A-Z]{2})/)))
        {
          var lookupValue = "LMD" + command;

          var mode = this.GetKeyByValue(this.cmds["SOUND_MODES"] , lookupValue);
          this.log("Sound mode: " + mode);
...
Onkyo.prototype.Connect = function(callback){
	...
	this.client.on('uncaughtException',function(error){
	   self.emit("error", error);
	})
  }
...
Onkyo.prototype.GetKeyByValue = function(obj, value )
  {
