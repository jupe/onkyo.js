...
const eiscpCmds = require('./eiscp-commands.json');
...
class Onkyo extends EventEmitter {
  ...
  _parseMsg(lookup) {
    ...
    switch (group) {
      ...
      case 'NDN':
      case 'SAM': { // unknown command
        return Promise.resolve({group, data: {[group]: command}});
      }
      default:
        if (group in eiscpCmds) {
          const CMD = eiscpCmds[group];
          const data = CMD.values[command] ? CMD.values[command].name : command;
          this.logger.debug(`${CMD.name}: ${data}`);
          return Promise.resolve({group, data: {[group]: data}});
        }
        break;
    }
