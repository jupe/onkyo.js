...
class Onkyo extends EventEmitter {
  ...
  _parseMsg(lookup) {
    ...
    if (command === 'N/A') {
      const error = new OnkyoError('Onkyo does not support this command');
  ...
  pwrState() {
    return this.sendCommand('POWER', 'STATUS');
  }
  pwrOn() {
    return this.pwrState()
      .then(({PWR}) => {
  ...
  muteState() {
    return this.sendCommand('AUDIO', 'STATUS_MUTE');
  }
  sourceState() {
    return this.sendCommand('SOURCE_SELECT', 'STATUS');
  }
  soundModeState() {
  ...
  getVolume() {
    return this.sendCommand('AUDIO', 'STATUS')
  ...
  setVolume(volume) {
    ...
    const volumeHex = volume.toString(16);
