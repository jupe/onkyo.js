...
class Onkyo extends EventEmitter {
  ...
  _parseMsg(lookup) {
    ...
    // always resolve even if data is unknown
  ...
  volUp(zone) {
    return this.sendCommand(OnkyoCmds.getZoneCommand('AUDIO', zone), 'VOL_UP');
  }
  volDown(zone) {
    return this.sendCommand(OnkyoCmds.getZoneCommand('AUDIO', zone), 'VOL_DOWN');
  }
