...
class Onkyo extends EventEmitter {
  constructor(options) {
    super();
    this.collectUnrecognizedToFile = _.get(
      options,
      'collectUnrecognizedToFile', false
    );
  ...
  _generateApis() {
    ...
    _.each(groups, (group) => {
      ...
      _.each(cmds, (cmd, key) => {
        const api = _.camelCase(`set${key}`);
        const f = () => this.sendCommand(group, key);
        _.set(this, api, f);
        apis.push({title: key, api});
      });
    });
    apis.push({title: 'power Toggle', api: this.pwrToggle.bind(this)});
    return apis;
  }
  validate() {
  ...
  _parseMsg(msg) {
    ...
    this.logger.debug(`msg: ${group} ${command}`);
    switch (group) {
      case ('TUN'):
      case ('SPA'):
      case ('LMD'):
  ...
  static eiscpPacketExtract(packet) {
    ...
    */
    ...
    data = data.match(/(.*)[\x1a\x0d\x0a\x00]{1,}$/);
    if (!data) {
      throw new Onkyo(`eiscp packet malformed: ${encodeURIComponent(packet)}`);
    }
    [, data] = data;
    return data;
  }
  ...
  sendCommand(group, cmd) {
    const sendPromise = () => new Promise((resolve) => {
      if (!_.has(OnkyoCmds, `${group}.${cmd}`)) {
        throw new OnkyoError(`Command '${group}:${cmd}' not supported`);
      }
